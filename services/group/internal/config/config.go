...
// Config 保存服务级配置，包括监听地址、etcd 端点和数据库连接信息。
type Config struct {
	...
	GroupInfoCacheTTL time.Duration `yaml:"group_info_cache_ttl"`
}
...
func defaults() *Config {
	return &Config{
		...
		RedisDB:           0,
		GroupInfoCacheTTL: 12 * time.Hour,
	}
}
...
func LoadFromFile(path string) *Config {
	...
	if data, err := os.ReadFile(path); err == nil {
		if err := yaml.Unmarshal(data, cfg); err != nil {
			slog.Warn("[group] failed to parse config file, using defaults", "path", path, "error", err)
		}
	} else {
		slog.Warn("[group] config file not found, using defaults", "path", path)
	}
	...
	if v := os.Getenv("GROUP_INFO_CACHE_TTL"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.GroupInfoCacheTTL = d
		}
	}
	return cfg
}
