...
import { Search, UserPlus, Check, Clock, Loader2, ArrowLeft } from "lucide-react";
...
export default function AddFriendPanel({ embedded = false, onBack }: AddFriendPanelProps) {
    ...
    <div className={embedded ? "h-full flex flex-col bg-white" : "h-full flex flex-col bg-white"}>
      ...
      {embedded && onBack && (
        <button onClick={onBack}
          className="flex items-center gap-1.5 px-4 py-2.5 text-sm text-gray-500 hover:text-indigo-500 transition-colors duration-200 w-full border-b border-gray-50">
          <ArrowLeft className="w-4 h-4" /><span>返回联系人</span>
        </button>
      )}
      {/* 头部 — 仅在独立模式显示 */}
      {!embedded && (
