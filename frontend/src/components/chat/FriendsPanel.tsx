...
export default function FriendsPanel() {
  const { contacts, conversations, setActiveConversation, refreshConversations, searchContacts } = useChat();
    ...
    <div className="h-full flex flex-col bg-white w-full">
      ...
      {subView === "requests" && (
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            <FriendRequestPanel embedded onHandled={fetchBadge} />
          </div>
        </div>
      )}
...
// ===== 好友请求管理（嵌入版） =====
function FriendRequestPanel({ embedded, onHandled }: { embedded?: boolean; onHandled?: () => void }) {
  ...
  }, [handlingId, refreshConversations, onHandled]);
