...
func LoadFromFile(path string) *Config {
	...
	if data, err := os.ReadFile(path); err == nil {
		if err := yaml.Unmarshal(data, cfg); err != nil {
			slog.Warn("[message] failed to parse config file, using defaults", "path", path, "error", err)
		}
	} else {
		slog.Warn("[message] config file not found, using defaults", "path", path)
	}
